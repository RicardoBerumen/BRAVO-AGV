...
with np.load("C:/BRAVO/coca-cola-can-detection.v1i.yolov12/camera_calibration2.npz") as X:
...
model = YOLO("C:/BRAVO/coca-cola-can-detection.v1i.yolov12/runs/train/EVO1/weights/best.pt")
# === Loop principal ===
while True:
    ret, frame = cap.read()
    if not ret:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if 0 <= cy2d < depth_data.shape[0] and 0 <= cx2d < depth_data.shape[1]:
                ...
                width_px = x2 - x1
                height_px = y2 - y1
                width_cm = (width_px * Z / fx) 
                height_cm = (height_px * Z / fy) 
                print(f"{width_cm:.2f}")
